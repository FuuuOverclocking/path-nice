...
async function move(path, fs, src, dest, options) {
    ...
    function doRename(src, dest, overwrite, isChangingCase) {
        ...
        if (overwrite) {
            return (0, remove_js_1.remove)(fs, dest).then(() => _rename(src, dest, overwrite));
        }
    ...
    async function moveAcrossDevice(src, dest, overwrite) {
        await (0, copy_js_1.copy)(path, fs, src, dest, {
            force: overwrite,
            errorOnExist: true,
        });
        await (0, remove_js_1.remove)(fs, src);
    }
    async function checkPaths(src, dest, opts) {
        ...
        if (destStat) {
            if (areIdentical(srcStat, destStat)) {
                ...
                throw new Error('[path-nice] .moveTo: src and dest must not be the same.');
            }
            if (srcStat.isDirectory() && !destStat.isDirectory()) {
                throw new Error(`[path-nice] .moveTo: cannot overwrite non-directory ${dest} with directory ${src}.`);
            }
            if (!srcStat.isDirectory() && destStat.isDirectory()) {
                throw new Error(`[path-nice] .moveTo: cannot overwrite directory '${dest}' with non-directory '${src}'.`);
            }
        }
        if (srcStat.isDirectory() && isSrcSubdir(src, dest)) {
            throw new Error(`[path-nice] .moveTo: cannot move ${src} to a subdirectory of self ${dest}`);
        }
    ...
    async function checkParentPaths(src, srcStat, dest) {
        ...
        if (areIdentical(srcStat, destStat)) {
            throw new Error(`[path-nice] .moveTo: cannot copy ${src} to a subdirectory of self ${dest}`);
        }
